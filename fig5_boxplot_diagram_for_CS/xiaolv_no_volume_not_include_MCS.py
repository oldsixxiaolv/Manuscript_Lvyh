...
for i in all_list[1:]:
    ...
    zuizhongy.reverse()
    xxxx = "Convective precipitation ratio"
    # 创建一个DataFrame对象开始绘图
    df = pd.DataFrame({name[k]: zuizhongy, xxxx: rx})
    # 求平均值
    all = df.groupby(xxxx, as_index=False).mean()
    di = 0.195
    r_threshold = 40
 ...
 # 利用以下的代码可以对plt的图中进行x轴或者y轴标签或者刻度进行字体等的设置
    if k == 0:
        sns.boxplot(x=xxxx, y=name[k], ax=ax, positions=[i - di for i in range(0, r_threshold)],
        ...
        ax1 = ax.twinx()
        seanborn =  sns.boxplot(x=xxxx, y=name[k], ax=ax1, positions=[j + di for j in range(0, r_threshold)],
    ...
    k += 1
plt.savefig(f"/root/git/Project_develop/figures/Appendix_maxht_volume_fls_boxplot_newest_not_include_MCS_no_Volume.jpeg",
