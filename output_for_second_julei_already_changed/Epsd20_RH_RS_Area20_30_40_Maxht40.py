...
def find_best_eps(X, min_pts, plot=False):
    ...
    knee_idx = curv_sort[-1300]
    best_eps = k_dist[curv_sort[-1300]]
    if plot:
...
X = np.array(list(zip(ellip_20, maxht20_minus_maxht40_divide_maxht20, npx40_divide_npx20, npixels_20, npixels_30, npixels_40, maxht40
# Volume40_divide_20
                      ))).reshape(len(ellip_20), 7)
# print('各维度极差:', np.ptp(X, axis=0))
# print('各维度 std :', X.std(axis=0))
# print('各维度均值:', X.mean(axis=0))
best_eps = find_best_eps(X, 8)
print(best_eps)
...
for k in K:
    ...
    kmeanModel = BisectingKMeans(n_clusters=k, init="k-means++", n_init=10, max_iter=1000 , tol=1e-5).fit(core_samples)
    labels = kmeanModel.labels_
    print(len(labels))
    cluster_centers = feature_normalize_back(kmeanModel.cluster_centers_, data_last[13], data_last[12]/data_last[1], data_last[18], data_last[4], data_last[5], data_last[6], data_last[3]
#data_last[24]/data_last[23]
    ...
    if k == 1:
        ...
        print(f"k={k}")
        print("DBI", dbi)
        print("CH", round(float(metrics.calinski_harabasz_score(core_samples, kmeanModel.labels_))))
        print("Inertias", kmeanModel.inertia_)
...
# plt.savefig("/root/git/Project_develop/manuscript_review/figures/zhoubutu.jpeg", bbox_inches="tight", dpi=400)
