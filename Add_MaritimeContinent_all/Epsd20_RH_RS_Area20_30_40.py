...
def find_best_eps(X, min_pts, plot=True):
    ...
    knee_idx = curv_sort[-920]
    best_eps = k_dist[curv_sort[-920]]
    if plot:
        ...
        plt.savefig("/home/lvyh/git/Project/figures/K_distance.jpeg", bbox_inches="tight", dpi=400)
...
X = np.array(list(zip(ellip_20, maxht20_minus_maxht40_divide_maxht20, npx40_divide_npx20, npixels_20, npixels_30, npixels_40
# Volume40_divide_20
                      ))).reshape(len(ellip_20), 6)
# print('各维度极差:', np.ptp(X, axis=0))
# print('各维度 std :', X.std(axis=0))
# print('各维度均值:', X.mean(axis=0))
best_eps = find_best_eps(X, 7)
print(best_eps)
...
for k in K:
    ...
    print(len(labels))
    cluster_centers = feature_normalize_back(kmeanModel.cluster_centers_, data_last[13], data_last[12]/data_last[1], data_last[18], data_last[4], data_last[5], data_last[6]
#data_last[24]/data_last[23]
    ...
    if k == 1:
        ...
        print("Volume20", other_parameters(labels, k, data_last[23], core_samples_mask))
        print("Volume40", other_parameters(labels, k, data_last[24], core_samples_mask))
