...
def find_best_eps(X, min_pts, plot=False):
    ...
    knee_idx = curv_sort[-930]
    best_eps = k_dist[curv_sort[-930]]
    if plot:
...
# print(sorted(list(map(float, ellip_20_maxht20))))
...
X = np.array(list(zip(ellip_20_maxht20,  maxht20_minus_maxht40_divide_maxht20, npx40_divide_npx20, ellip_20
# Volume40_divide_20
                      ))).reshape(len(ellip_20), 4)
# print('各维度极差:', np.ptp(X, axis=0))
# print('各维度 std :', X.std(axis=0))
# print('各维度均值:', X.mean(axis=0))
best_eps = find_best_eps(X, 6)
print(best_eps)
...
K = range(1, 8)
for k in K:
    dbscan = DBSCAN(eps=best_eps, min_samples=6)
    labels = dbscan.fit_predict(X)
    core_samples_mask = np.zeros_like(dbscan.labels_, dtype=bool)
    core_samples_mask[dbscan.core_sample_indices_] = True
    core_samples = X[core_samples_mask]
    # print(len(core_samples))
    kmeanModel = KMeans(n_clusters=k, init="k-means++", n_init=100, max_iter=10000 , tol=1e-8).fit(core_samples)
    labels = kmeanModel.labels_
    print(len(labels))
    cluster_centers = feature_normalize_back(kmeanModel.cluster_centers_, data_last[20], data_last[12]/data_last[1], data_last[18], data_last[13]
