...
def find_best_eps(X, min_pts, plot=False):
    ...
    knee_idx = curv_sort[-949]
    best_eps = k_dist[curv_sort[-949]]
    if plot:
...
print(ellip_30_maxht30)
print("mean", np.median(ellip_30_maxht30))
print("min", np.min(ellip_30_maxht30))
print("max", np.max(ellip_30_maxht30))
...
X = np.array(list(zip(ellip_30_maxht30, maxht20_minus_maxht40_divide_maxht20, npx40_divide_npx20, 
# Volume40_divide_20
                      ))).reshape(len(ellip_30), 3)
...
for k in K:
    ...
    kmeanModel = KMeans(n_clusters=k, init="k-means++").fit(core_samples)
    labels = kmeanModel.labels_
    print(len(labels))
    cluster_centers = feature_normalize_back(kmeanModel.cluster_centers_, data_last[21], data_last[12]/data_last[1], data_last[18] 
#data_last[24]/data_last[23]
    ...
    kmean2_DBI = KMeans(n_clusters=k, init="k-means++").fit_predict(core_samples)
...
plt.savefig("/root/git/Project_develop/manuscript_review/figures/zhoubutu_Epsd30_Maxht30_RH_RS.jpeg", bbox_inches="tight", dpi=400)
