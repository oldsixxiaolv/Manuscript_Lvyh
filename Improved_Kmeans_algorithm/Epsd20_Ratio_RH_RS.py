...
def find_best_eps(X, min_pts, plot=False):
    ...
    knee_idx = curv_sort[-910]
    best_eps = k_dist[curv_sort[-910]]
    if plot:
...
X = np.array(list(zip(ellip_20, maxht20_minus_maxht40_divide_maxht20, npx40_divide_npx20, r
# Volume40_divide_20
...
for k in K:
    ...
    print(len(labels))
    cluster_centers = feature_normalize_back(kmeanModel.cluster_centers_, data_last[13], data_last[12]/data_last[1], data_last[18], data_last[0]
...
df = DataFrame({"r": r[core_samples_mask], "maxht20": maxht20[core_samples_mask],
                ...
                "labels": labels})
df.to_excel(r"./ouput_for_julei_Ratio_Epsd20_RH_RS.xlsx", sheet_name="sheet1", index=False)
