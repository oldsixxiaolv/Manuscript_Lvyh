...
def _contour_(ax, x, y, x_max, y_max, x_minus, y_minus, levels):
    ...
    Z = kde(grid_coords).reshape(X.shape) * len(x)
    axx = ax.contour(X, Y, Z, colors="black", extend="both", linewidths=5, levels=levels)
    plt.clabel(axx, inline=True, colors="#FFC75F", fontsize=37)
