...
name = ["Maxht20 (km)", "Maxht30 (km)", "Maxht40 (km)", r'FlRate (fl' + r'$\cdot$' + r'min$\mathregular{^{-1}}$)',
        r"R_eq20 (km$\mathregular{^{2}}$)",
        r"R_eq30 (km$\mathregular{^{2}}$)", r"R_eq40 (km$\mathregular{^{2}}$)",
...
for j in range(8, 9):
    ...
    ax.set_xlabel("Convective precipitation ratio", fontsize=75)
    xticks = ax.xaxis.get_major_ticks()
    for i in range(0, 101, 5):
        xticks[i].tick1line.set_markersize(15)
    ax.set_xticklabels(labels, font={"family": "Times New Roman", "size": 55})
...
plt.savefig(f"/root/git/Project_develop/figures/fig5_Ratio_boxplot_Pre_Mature_Post.jpeg", bbox_inches="tight", dpi=400)
"""ax.text(
        0.2, 0.1, 'some text',
