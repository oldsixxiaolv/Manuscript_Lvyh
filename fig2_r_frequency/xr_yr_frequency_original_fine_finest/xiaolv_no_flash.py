...
from read_shuju_include_no_flash import read_shuju
...
ax.set_yticks(np.array(list(map(int, list(np.arange(0, 20000, 1000))))))
ax.set_yticklabels(np.array(list(map(int, list(np.arange(0, 20000, 1000)))))
                   , font={"family": "Times New Roman", "size": 55})
ax.bar(labels, frequency, width=0.006)
ax.set_ylim(-40, 20000)  # (-40, None)
...
# 储存绘制的图形
# 把ax2再画蓝线
ax2.legend(loc=(0.66, 0.1), prop={"family": "Times New Roman", "size": 63})
plt.savefig(r"/root/git/Project_develop/figures/xr_frequency_include_no_flash.jpeg", dpi=400)
