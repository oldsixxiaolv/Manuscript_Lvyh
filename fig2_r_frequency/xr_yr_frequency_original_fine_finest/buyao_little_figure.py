...
fig, ax2 = plt.subplots(1, 1, figsize=(60, 12))
...
ax2.set_ylabel("percent (%)", font={"family": "Times New Roman", "size": 70}, labelpad=25)
...
# 指明x和y的范围
ax2.text(0.01, 0.89, "(a)", transform=ax2.transAxes, fontsize=75)
ax2.set_ylim(-0.05, None)
# 让r的坐标从最左边的哪里开始出现标签
ax2.set_xlim(-0.01, 1.008)
...
"""从这里绘制第二层图形"""
ax = ax2.twinx()
ax.set_ylabel("Frequence (#)", font={"family": "Times New Roman", "size": 70}, labelpad=25)
...
ax1.set_yticklabels(np.array(list(map(round2, list(np.arange(0, 110, 20))))),
                    font={"family": "Times New Roman", "size": 80})
ax1.set_ylabel("CDF (%)", font={"family": "Times New Roman", "size": 70}, color="red", labelpad=25)
...
#             label=f"Ratio = {round(begin2, 2)} ,  {round(jiu2*100)}%", linewidth=10)
ax1.hlines(jiu2, 1-begin2, 1, linestyles="dashed", color="black",
            label=f"Ratio = {round(begin2, 2)} ,  {round(jiu2*100)}%", linewidth=10)
# ax1.tick_params(axis="y", direction='in', which="minor", length=12, width=2)
# 储存绘制的图形
# 把ax2再画蓝线
ax2.legend(loc=(0.74, 0.708), prop={"family": "Times New Roman", "size": 70})
# 嵌入局部放大图坐标系
...
plt.savefig(r"/root/git/Project_develop/figures/fig2_xr_frequency_delete_num_less_100_buyao_little_figure.jpeg", bbox_inches='tight', dpi=400)
