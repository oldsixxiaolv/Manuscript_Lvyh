...
gs = GridSpec(112, 88, figure=fig)
# abc = ["(a)", "(b)", "(c)"]
index = 0
for i in All_left_right:
    # 一定要记住的是tickparams中left是调整刻度线，labelleft才是调整刻度标签
    if index==0:
        ax = fig.add_subplot(gs[52:112, 0:30])
        ax_violin = fig.add_subplot(gs[0:43, 0:26])
        ...
        ax.set_ylabel("Altitude (km)", fontsize=45)
        # 20dBZ
        ax.fill_betweenx(Height_n, i[0][0], i[1][0], hatch="|", color="#4DAF4A")
        # 30dBZ
        ax.fill_betweenx(Height_n, i[0][1], i[1][1], hatch="|", color="#FFA500")
        # 40dBZ
        ax.fill_betweenx(Height_n, i[0][2], i[1][2], hatch="|", color="#E41A1C")
    elif index==1:
        ax = fig.add_subplot(gs[52:112, 29:59])
        ax_violin = fig.add_subplot(gs[0:43, 31:57])
        ax_violin2 = ax_violin.twinx()
        ax_violin.tick_params(axis="both", direction='out', length=10, width=2)
        ax_violin2.tick_params(axis="both", direction='out', length=10, width=2)
        ax.tick_params(axis="both", direction='out', length=10, width=2,
                    top=False, right=False, left=False, labelleft=False)
        ax.set_xlabel("Horizontal scale (km)", fontsize=45, labelpad=20)
        # 20dBZ
        ax.fill_betweenx(Height_n, i[0][0], i[1][0], hatch="|", color="#4DAF4A")
        # 30dBZ
        ax.fill_betweenx(Height_n, i[0][1], i[1][1], hatch="|", color="#FFA500")
        ...
        ax = fig.add_subplot(gs[52:112, 58:88])
        ax_violin = fig.add_subplot(gs[0:43, 62:88])
        ax_violin2 = ax_violin.twinx()
        ax_violin.tick_params(axis="both", direction='out', length=10, width=2)
        ax_violin2.tick_params(axis="both", direction='out', length=10, width=2)
        ax.tick_params(axis="both", direction='out', length=10, width=2,
                    top=False, right=False, left=False, labelleft=False)
        # 20dBZ
        ax.fill_betweenx(Height_n, i[0][0], i[1][0], hatch="|", color="#4DAF4A", label="20 dBZ")
        # 30dBZ
        ax.fill_betweenx(Height_n, i[0][1], i[1][1], hatch="|", color="#FFA500", label="30 dBZ")
        # 40dBZ
        ax.fill_betweenx(Height_n, i[0][2], i[1][2], hatch="|", color="#E41A1C", label="40 dBZ")
    ...
    #         baby += 1    
    ax.xaxis.set_major_locator(FixedLocator(np.arange(0, 45.5, 5)))
    ...
    ax_violin.set_ylim(-4, 41.75)
    ax_violin.set_yticks([0, 10, 20, 30, 40])
    ax_violin2.set_ylim(-1.15, 12.5)
    ax_violin2.set_yticks([0, 3, 6, 9, 12])
    # ax_violin.spines["top"].set_visible(False)
    # ax_violin2.spines["top"].set_visible(False)
    ax_violin.tick_params(axis='y', labelcolor='#7030A0')
    ax_violin2.tick_params(axis='y', labelcolor='#BC9A6A')
    ...
    print(len(Flashrate_all[index]))
    print(len(Fl40_all[index]))
    data_ = pd.DataFrame({"Group": len(Flashrate_all[index]) * ["FlRate"] + len(Fl40_all[index]) * ["FD$_{40}$"], "Value1":np.array(2 * list(Flashrate_all[index])), "Value2":np.array(2 * list(Fl40_all[index]))})
    # 在小提琴图第一个量上面叠加一个sns.boxplot
    sns.violinplot(x=np.zeros(len(Flashrate_all[index])), y='Value1', data=data_[data_["Group"] == "FlRate"], 
                          ax=ax_violin, color='#7030A0', width=0.8,  linewidth=3, saturation=0.6, cut=0, bw_adjust=1.5, inner_kws=dict(box_width=15, whis_width=5, color="k"), inner=None)
    sns.boxplot(
        data=data_[data_["Group"] == "FlRate"], x=np.zeros(len(Flashrate_all[index])), y='Value1', whis=(5, 95), showfliers=False, color="k",
                    linewidth=3,
        width=0.03,               # 足够窄，不会挡住 violin
        boxprops={'zorder': 2},   # 确保在上层
        ...
        ax=ax_violin)
    # 叠加sns.boxplot到第二个量上面
    sns.violinplot(x=np.ones(len(Fl40_all[index])), y='Value2', data=data_[data_["Group"] == "FD$_{40}$"],
                          ax=ax_violin2, color='#BC9A6A', width=0.8, linewidth=3, saturation=0.6, cut=0, bw_adjust=1.5, inner_kws=dict(box_width=15, whis_width=5, color="k"), inner=None)
    sns.boxplot(
        data=data_[data_["Group"] == "FD$_{40}$"], x=np.ones(len(Fl40_all[index])), y='Value2', whis=(5, 95), showfliers=False, color="k",
                    linewidth=3,
        width=0.03,               # 足够窄，不会挡住 violin
        boxprops={'zorder': 2},   # 确保在上层
        capprops={'linewidth': 2},
...
plt.savefig(f"/root/git/Project_develop/figures/fig10_violin.jpeg", bbox_inches="tight", dpi=400)
"""ax.text(
        0.2, 0.1, 'some text',
